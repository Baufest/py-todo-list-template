...
This project is meant to run tests for automation.  
...
You can execute tests from your local machine against Selenium Standlone or directly against Chromedriver.

---

__Selenium Standalone, the option by default__

By just running *'npm run test'* on your terminal, a Selenium server starts by default on localhost:4444 and the tests start to be executed against the browsers configured in the capability settings declared in your config files.

In order to execute tests against browsers, it's important to note that the drivers for such browsers must be available in your project. By default, Acis provides support for Chrome, Firefox, Edge and Safari, so in most cases you shouldn't need to do an extra effort to get them installed:

--

<u>Firefox, Microsoft Edge & Safari</u>

By default, drivers for the latest versions of Firefox, Microsoft Edge and Safari try to be downloaded. In case you need to execute tests against an older version, you can fix the version of the driver via environment variable:
...
The driver used for Google Chrome will be by default the version detected in the operating system during the installation of the testing project with Acis.

Accordingly, each time a Google Chrome version is updated in your operating system, you need to run *'npm i'* in your testing project, so that drivers can be updated too.

In case you have more than one Chrome installation and need to force the download of the driver for that specific version, you can use the CHROME_VERSION environment variable in order to fix the version:

```bash
    export CHROME_VERSION=114.0.0
```
...
If you just need to execute tests against Google Chrome, we offer the option of skipping Selenium Standalone server so that tests can be executed directly against a Chromedriver running on localhost:9515. For that you just need to follow these steps:

- add [chromedriver](https://www.npmjs.com/package/chromedriver) and [wdio-chromedriver-service](https://www.npmjs.com/package/wdio-chromedriver-service) dependencies to your package.json

- set the LAUNCH_CHROMEDRIVER environment variable to truthy:

```bash
    export LAUNCH_CHROMEDRIVER=true
    npm t
```  

---

#### 3.2 Remote Browsers / Mobile Platforms Providers

Acis integrates the option of running tests in remote browsers / mobile platforms with two providers: Galatea and Sauce Labs.
...
This scaffold comes with a Jenkinsfile that allows us to execute tests in Jenkins via browsers / mobile platforms provider (Galatea by default). Check the Jenkinsfile to read instructions on how to get the pipeline working.
...
```bash
    export DATAMANAGER_BASE_URL=https://datamanager.globaldevtools.bbva.com/data-manager-api/namespaces/your_namespace
```

---

#### 3.5 Executing tests under PROXY
